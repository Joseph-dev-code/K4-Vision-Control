...
# Tolerance level for considering distance as equal to 20 cm
tolerance = 0.1
...
while cap.isOpened():
    ...
    for box in results[0].boxes:
        ...
        if object_height_pixels > 0 and class_id in objects_height_cm:
            ...
            # If the distance is close to 20 cm, send the prediction
            if abs(distance - 20) < tolerance and time.time() - last_prediction_time[class_id] >= cooldown_period:
                print(f"[DEBUG] Sending prediction for {class_id}")
